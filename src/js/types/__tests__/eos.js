...
export const eosGetPublicKey = async () => {
    // regular
    const singlePK = await JuBiterConnect.eosGetPublicKey({ path: 'm/44' });
    ...
    // bundle
    const bundlePK = await JuBiterConnect.eosGetPublicKey({ bundle: [{ path: 'm/44' }] });
    ...
    const e1 = await JuBiterConnect.eosGetPublicKey({ path: 'm/44' });
    if (e1.success) {
    ...
    // $FlowExpectedError: payload is Address[]
    const e2 = await JuBiterConnect.eosGetPublicKey({ bundle: [{ path: 'm/44' }] });
...
export const eosSignTransaction = async () => {
    ...
    const sign = await JuBiterConnect.eosSignTransaction({
        path: "m/44'/194'/0'/0/0",
        transaction: {
